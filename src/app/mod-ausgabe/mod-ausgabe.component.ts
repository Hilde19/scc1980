...
@Component({
  selector: 'app-mod-ausgabe',
  templateUrl: './mod-ausgabe.component.html',
  styleUrls: ['./mod-ausgabe.component.css']
})
export class ModAusgabeComponent implements OnInit {

    @Input() ausgabekennzeichen: string = '';  
    expenses: Expense[]; 
        ...
        this._ausgabenService.getExpenses().subscribe(
          data => {
            ...
            data.forEach(element => {
             if(element.ausgabekennz === this.ausgabekennzeichen){ 
              if (s_bestell === 0){
